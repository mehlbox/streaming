...
const scheduleBaseUrl = (document.body?.dataset?.scheduleBaseUrl || "").replace(/\/$/, "");
const scheduleUrl = document.body?.dataset?.scheduleUrl || (scheduleBaseUrl ? `${scheduleBaseUrl}/schedule-${scheduleTheme}.json` : "");
...
const renderSatelliteTable = (satellites) => {
  ...
  satelliteBody.innerHTML = satellites.map((sat) => {
    ...
    const maxBandwidth = scalewayBandwidth;
    const maxBandwidthLabel = `(${maxBandwidth.toFixed(1)} Mbps)`;
